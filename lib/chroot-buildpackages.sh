		...
		for arch in $target_arch; do
			...
			for plugin in "${SRC}"/packages/extras-buildpkgs/*.conf; do
				...
				eval systemd-nspawn -a -q \
					...
					${PROGRESS_LOG_TO_FILE:+' | tee -a $DEST/$(get_log_path)/buildpkg.log'}
...
chroot_installpackages_local()
{
	...
	aptly -config="${conf}" repo create temp >> "${DEST}"/$(get_log_path)/install.log
	# NOTE: this works recursively
	aptly -config="${conf}" repo add temp "${DEB_STORAGE}/extra/${RELEASE}-desktop/" >> "${DEST}"/$(get_log_path)/install.log
	aptly -config="${conf}" repo add temp "${DEB_STORAGE}/extra/${RELEASE}-utils/" >> "${DEST}"/$(get_log_path)/install.log
	# -gpg-key="925644A6"
	aptly -keyring="${SRC}/packages/extras-buildpkgs/buildpkg-public.gpg" -secret-keyring="${SRC}/packages/extras-buildpkgs/buildpkg.gpg" -batch=true -config="${conf}" \
		 -gpg-key="925644A6" -passphrase="testkey1234" -component=temp -distribution="${RELEASE}" publish repo temp >> "${DEST}"/$(get_log_path)/install.log
...
chroot_installpackages()
{
	...
	chroot "${SDCARD}" /bin/bash -c "/tmp/install.sh" >> "${DEST}"/$(get_log_path)/install.log 2>&1
} #############################################################################

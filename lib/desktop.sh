...
create_desktop_package ()
{
	...
	echo "Showing PACKAGE_LIST_DESKTOP before postprocessing" >> "${DEST}"/$(get_log_path)/output.log
	# Use quotes to show leading and trailing spaces
	echo "\"$PACKAGE_LIST_DESKTOP\"" >> "${DEST}"/$(get_log_path)/output.log
	...
	echo "DEBIAN_RECOMMENDS : ${DEBIAN_RECOMMENDS}" >> "${DEST}"/$(get_log_path)/output.log
	...
	echo "${PACKAGE_LIST_PREDEPENDS}" >> "${DEST}"/$(get_log_path)/output.log
	...
	cat "${destination}/DEBIAN/postinst" >> "${DEST}"/$(get_log_path)/install.log
...
add_desktop_package_sources() {
	...
	ls -l "${SDCARD}/etc/apt/sources.list.d" >> "${DEST}"/$(get_log_path)/install.log
	cat "${SDCARD}/etc/apt/sources.list" >> "${DEST}"/$(get_log_path)/install.log

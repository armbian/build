...
# build aarch64
	...
	echo -e "\n\t==  atf  ==\n" >> "${DEST}"/$(get_log_path)/compilation.log
	...
	eval CCACHE_BASEDIR="$(pwd)" env PATH="${toolchain}:${toolchain2}:${PATH}" \
		...
		CROSS_COMPILE="$CCACHE $ATF_COMPILER"' 2>> "${DEST}"/$(get_log_path)/compilation.log \
		${PROGRESS_LOG_TO_FILE:+' | tee -a $DEST/$(get_log_path)/compilation.log'} \
...
# build aarch64
	...
	while read -r target; do
		...
		eval CCACHE_BASEDIR="$(pwd)" env PATH="${toolchain}:${toolchain2}:${PATH}" \
			...
			CROSS_COMPILE="$CCACHE $UBOOT_COMPILER"' 2>> "${DEST}"/$(get_log_path)/compilation.log \
			${PROGRESS_LOG_TO_FILE:+' | tee -a $DEST/$(get_log_path)/compilation.log'} \
			${OUTPUT_VERYSILENT:+' >/dev/null 2>/dev/null'}
		...
		eval CCACHE_BASEDIR="$(pwd)" env PATH="${toolchain}:${toolchain2}:${PATH}" \
			...
			"${cross_compile}"' 2>>"${DEST}"/$(get_log_path)/compilation.log \
			${PROGRESS_LOG_TO_FILE:+' | tee -a "${DEST}"/$(get_log_path)/compilation.log'} \
	...
	fakeroot dpkg-deb -b "$uboottempdir/${uboot_name}" "$uboottempdir/${uboot_name}.deb" >> "${DEST}"/$(get_log_path)/output.log 2>&1
	rm -rf "$uboottempdir/${uboot_name}"
...
# build aarch64
	...
	eval CCACHE_BASEDIR="$(pwd)" env PATH="${toolchain}:${PATH}" \
		...
		$KERNEL_IMAGE_TYPE modules dtbs 2>>$DEST/$(get_log_path)/compilation.log' \
		${PROGRESS_LOG_TO_FILE:+' | tee -a $DEST/$(get_log_path)/compilation.log'} \
	...
	eval CCACHE_BASEDIR="$(pwd)" env PATH="${toolchain}:${PATH}" \
		...
		CROSS_COMPILE="$CCACHE $KERNEL_COMPILER" 2>>$DEST/$(get_log_path)/compilation.log' \
		${PROGRESS_LOG_TO_FILE:+' | tee -a $DEST/$(get_log_path)/compilation.log'} \
...
compile_firmware()
{
	...
	fakeroot dpkg -b "armbian-firmware${FULL}_${REVISION}_all" >> "${DEST}"/$(get_log_path)/install.log 2>&1
...
find_toolchain()
{
	...
	if [[ -f "${DEST}"/$(get_log_path)/compiler.log ]]; then
		if ! grep -q "$toolchain" "${DEST}"/$(get_log_path)/compiler.log; then
			echo "$toolchain" >> "${DEST}"/$(get_log_path)/compiler.log;
		fi
	else
			echo "$toolchain" >> "${DEST}"/$(get_log_path)/compiler.log;
	fi
...
process_patch_file()
{
	...
	echo "Processing file $patch" >> "${DEST}"/$(get_log_path)/patching.log
	patch --batch --silent -p1 -N < "${patch}" >> "${DEST}"/$(get_log_path)/patching.log 2>&1
	...
	echo >> "${DEST}"/$(get_log_path)/patching.log
}

...
install_common()
{
	...
	if [[ $CRYPTROOT_ENABLE == yes && $CRYPTROOT_SSH_UNLOCK == yes ]]; then
		...
		if [[ -f $USERPATCHES_PATH/dropbear_authorized_keys ]]; then
			...
			CRYPTROOT_SSH_UNLOCK_KEY_NAME="Armbian_${REVISION}_${BOARD^}_${RELEASE}_${DESKTOP_ENVIRONMENT}_${BRANCH}_${VER/-$LINUXFAMILY/}".key
	...
	if [[ "${REPOSITORY_INSTALL}" != *armbian-desktop* ]]; then
		...
		if [[ $BUILD_DESKTOP == yes ]]; then
			display_alert "Installing from repository" "${CHOSEN_DESKTOP}"
			chroot "${SDCARD}" /bin/bash -c "apt-get -y -qq install ${CHOSEN_DESKTOP}" >> "${DEST}"/debug/install.log 2>&1
			# install display manager and PACKAGE_LIST_DESKTOP_FULL packages if enabled per board
			desktop_postinstall
		fi
	fi

...
install_deb_chroot()
{
	...
	chroot "${SDCARD}" /bin/bash -c "DEBIAN_FRONTEND=noninteractive apt-get -yqq $apt_extra --no-install-recommends install $name" >> "${DEST}"/$(get_log_path)/install.log 2>&1
...
run_on_sdcard()
{
	...
	chroot "${SDCARD}" /bin/bash -c "${@}" >> "${DEST}"/$(get_log_path)/install.log

...
# create_new_rootfs_cache_via_debootstrap creates a root filesystem in SDCARD using mmdebstrap-based provisioning, configures locales and apt sources, installs aggregated packages (optionally desktop packages), performs chroot setup and cleanup (policy diverts, qemu handling, resolvconf, machine-id masking), and prepares the rootfs cache for packaging.
function create_new_rootfs_cache_via_debootstrap() {
	...
	declare -a debootstrap_arguments=(

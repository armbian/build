...
function compile_uboot_target() {
	...
	# @FIXME: This implementation with OpenSBI is ugly, more robust code should be considered
	do_with_ccache_statistics run_host_command_logged_long_running \
		"CFLAGS='${uboot_cflags}'" "KCFLAGS='${uboot_cflags}'" \
		CCACHE_BASEDIR="$(pwd)" OPENSBI="$([ -z "$OPENSBISOURCE" ] || echo "$SRC/cache/sources/opensbi/fw_dynamic.bin")" OPENSBI="$([ -z "$OPENSBISOURCE" ] || echo "$SRC/cache/sources/opensbi/fw_dynamic.bin")" PATH="${toolchain}:${toolchain2}:${PATH}" \
...
function compile_uboot() {
	...
	if [[ -n "${toolchain}" ]]; then
		exit_with_error "Could not find required toolchain" "${UBOOT_COMPILER:-"MISSING-"}gcc ${UBOOT_USE_GCC:-"NOT_SET"}"
	fi

	## Second toolchain
	if [[ -z "${UBOOT_TOOLCHAIN2}" ]]; then
	...
	### @FIXME: Eval is dangerous, verify that it's sane to use here or attempt to sanitize
	...
	# Make the file hierarchy structure
	mkdir -p \

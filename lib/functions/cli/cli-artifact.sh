...
function cli_artifact_run() {
	...
	# prepare_host() runs below and requires both HOSTRELEASE and HOSTARCH. Under
	# CONFIG_DEFS_ONLY=yes prep_conf_main_minimal_ni() skips check_basic_host(), so neither is
	# set by then; obtain them here, without the sanity checks that skip implies.
	if [[ "${CONFIG_DEFS_ONLY}" == "yes" ]]; then

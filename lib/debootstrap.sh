...
debootstrap_ng()
{
	...
	chroot $SDCARD /bin/bash -c "dpkg --get-selections" | grep -v deinstall | awk '{print $1}' | cut -f1 -d':' > $DEST/$(get_log_path)/installed-packages-${RELEASE}$([[ ${BUILD_MINIMAL} == yes ]] && echo "-minimal")$([[ ${BUILD_DESKTOP} == yes  ]] && echo "-desktop").list 2>&1
...
create_rootfs_cache()
{
	...
	if [[ -f $cache_fname && ! -f $cache_fname.aria2 ]]; then
		...
		eval 'debootstrap --variant=minbase --include=${DEBOOTSTRAP_LIST// /,} ${PACKAGE_LIST_EXCLUDE:+ --exclude=${PACKAGE_LIST_EXCLUDE// /,}} \
			...
			${PROGRESS_LOG_TO_FILE:+' | tee -a $DEST/$(get_log_path)/debootstrap.log'} \
		...
		eval 'LC_ALL=C LANG=C chroot $SDCARD /bin/bash -c "DEBIAN_FRONTEND=noninteractive apt-get -y -qq \
			...
			${PROGRESS_LOG_TO_FILE:+' >> $DEST/$(get_log_path)/debootstrap.log'} \
		...
		echo $freespace >> $DEST/$(get_log_path)/debootstrap.log
...
prepare_partitions()
{
	...
	if [[ -n $rootpart ]]; then
		...
		mkfs.${mkfs[$ROOTFS_TYPE]} ${mkopts[$ROOTFS_TYPE]} $rootdevice >> "${DEST}"/$(get_log_path)/install.log 2>&1
	...
	if [[ -n $bootpart ]]; then
		...
		mkfs.${mkfs[$bootfs]} ${mkopts[$bootfs]} ${LOOP}p${bootpart} >> "${DEST}"/$(get_log_path)/install.log 2>&1
		mkdir -p $MOUNT/boot/
...
update_initramfs()
{
	...
	chroot $chroot_target /bin/bash -c "$update_initramfs_cmd" >> $DEST/$(get_log_path)/install.log 2>&1
	display_alert "Updated initramfs." "for details see: $DEST/$(get_log_path)/install.log" "info"

	display_alert "Re-enabling" "initramfs-tools hook for kernel"
	chroot $chroot_target /bin/bash -c "chmod -v +x /etc/kernel/postinst.d/initramfs-tools" >> "${DEST}"/$(get_log_path)/install.log 2>&1
...
create_image()
{
	...
	if [[ $ROOTFS_TYPE != nfs ]]; then
		...
		rsync -aHWXh --exclude="/boot/*" --exclude="/dev/*" --exclude="/proc/*" --exclude="/run/*" --exclude="/tmp/*" \
			--exclude="/sys/*" --info=progress2,stats1 $SDCARD/ $MOUNT/ >> "${DEST}"/$(get_log_path)/install.log 2>&1
	else
	...
	if [[ $(findmnt --target $MOUNT/boot -o FSTYPE -n) == vfat ]]; then
		# fat32
		rsync -rLtWh --info=progress2,stats1 $SDCARD/boot $MOUNT >> "${DEST}"/$(get_log_path)/install.log 2>&1
	else
		# ext4
		rsync -aHWXh --info=progress2,stats1 $SDCARD/boot $MOUNT >> "${DEST}"/$(get_log_path)/install.log 2>&1
	fi

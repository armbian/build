...
if [ -f $USERPATCHES_PATH/REVISION ]; then
  REVISION=$(cat "${USERPATCHES_PATH}"/REVISION)"$SUBREVISION" # all boards have same revision
else
  REVISION=$(cat "${SRC}"/REVISION)"$SUBREVISION" # all boards have same revision
fi

...
compilation_prepare()
{
	...
	if linux-version compare $version ge 3.14 && [ "$EXTRAWIFI" == yes ]; then
		...
		# attach to specifics tag or branch
		local rtl8821cuver="branch:master"

		display_alert "Adding" "Wireless drivers for Realtek RTL8811CU and RTL8821C chipsets ${rtl8821cuver}" "info"

		fetch_from_repo "https://github.com/brektrou/rtl8821CU" "rtl8821cu" "${rtl8821cuver}" "yes"
		cd ${SRC}/cache/sources/${LINUXSOURCEDIR}
		rm -rf ${SRC}/cache/sources/${LINUXSOURCEDIR}/drivers/net/wireless/rtl8821cu
		mkdir -p ${SRC}/cache/sources/${LINUXSOURCEDIR}/drivers/net/wireless/rtl8821cu/
		cp -R ${SRC}/cache/sources/rtl8821cu/${rtl8821cuver#*:}/{core,hal,include,os_dep,platform,rtl8821c.mk} \
		${SRC}/cache/sources/${LINUXSOURCEDIR}/drivers/net/wireless/rtl8821cu

		# Makefile
		cp ${SRC}/cache/sources/rtl8821cu/${rtl8821cuver#*:}/Makefile \
		${SRC}/cache/sources/${LINUXSOURCEDIR}/drivers/net/wireless/rtl8821cu/Makefile
		cp ${SRC}/cache/sources/rtl8821cu/${rtl8821cuver#*:}/Kconfig \
		${SRC}/cache/sources/${LINUXSOURCEDIR}/drivers/net/wireless/rtl8821cu/Kconfig

		# Disable debug
		sed -i "s/^CONFIG_RTW_DEBUG.*/CONFIG_RTW_DEBUG = n/" ${SRC}/cache/sources/${LINUXSOURCEDIR}/drivers/net/wireless/rtl8821cu/Makefile
		...
		# Add to section Makefile
		echo "obj-\$(CONFIG_RTL8821CU) += rtl8821cu/" >> $SRC/cache/sources/${LINUXSOURCEDIR}/drivers/net/wireless/Makefile
		sed -i '/source "drivers\/net\/wireless\/ti\/Kconfig"/a source "drivers\/net\/wireless\/rtl8821cu\/Kconfig"' \
		$SRC/cache/sources/${LINUXSOURCEDIR}/drivers/net/wireless/Kconfig
	...
	if linux-version compare $version ge 5.5 && [ "$EXTRAWIFI" == yes ]; then
		...
		# Patch
		rm -rf ${SRC}/cache/sources/${LINUXSOURCEDIR}/drivers/net/wireless/rtl8189es/os_dep/linux/ioctl_cfg80211.c
	...
	if linux-version compare $version ge 5.5 && [ "$EXTRAWIFI" == yes ]; then
		...
		# Patch
#		process_patch_file "${SRC}/patch/misc/wireless-rtl8189fs.patch"                "applying"

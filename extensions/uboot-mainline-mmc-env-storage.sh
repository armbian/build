...
function extension_prepare_config__uboot_mainline_mmc_env_storage() {
	declare -g UBOOT_MMC_ENV_SKIP="${UBOOT_MMC_ENV_SKIP:-no}"
	...
	case "${UBOOT_MMC_ENV_MODE}" in
		hwpart)
			...
			if [[ "${UBOOT_MMC_ENV_HW_PARTITION}" == "1" ]]; then
				...
				# Any other hw partition keeps both copies in that one area, so they must not overlap (in either order).
				if [[ $((UBOOT_MMC_ENV_OFFSET_REDUND)) -lt $((UBOOT_MMC_ENV_OFFSET + UBOOT_MMC_ENV_SIZE)) && $((UBOOT_MMC_ENV_OFFSET_REDUND + UBOOT_MMC_ENV_SIZE)) -gt $((UBOOT_MMC_ENV_OFFSET)) ]]; then
					exit_with_error "${EXTENSION}: hw partition ${UBOOT_MMC_ENV_HW_PARTITION} keeps both env copies in one boot area" \
						"they would overlap (${UBOOT_MMC_ENV_OFFSET} and ${UBOOT_MMC_ENV_OFFSET_REDUND}, size ${UBOOT_MMC_ENV_SIZE}); use UBOOT_MMC_ENV_HW_PARTITION=1 for the boot0+boot1 split, or give non-overlapping offsets"
				fi
			fi
		...
		offset)
			...
			# Real interval-overlap test: nothing forces the redundant copy to sit above the primary one.
			if [[ $((UBOOT_MMC_ENV_OFFSET_REDUND)) -lt ${env_primary_end} && $((UBOOT_MMC_ENV_OFFSET_REDUND + UBOOT_MMC_ENV_SIZE)) -gt $((UBOOT_MMC_ENV_OFFSET)) ]]; then
...
function check_uboot_produced_binary_file__uboot_mainline_mmc_env_storage() {
	...
	# The env window starts at whichever copy is lower; the redundant one is not guaranteed to be above the primary.
	declare -i env_start=$((UBOOT_MMC_ENV_OFFSET))
	[[ $((UBOOT_MMC_ENV_OFFSET_REDUND)) -lt ${env_start} ]] && env_start=$((UBOOT_MMC_ENV_OFFSET_REDUND))

	if [[ ${binfile_end} -gt ${env_start} ]]; then
		exit_with_error "${EXTENSION}: '${base_binfile}' would overwrite the u-boot environment" \
			"it is written at $(printf "0x%X" "${write_offset}") and ends at $(printf "0x%X" "${binfile_end}"), past the env starting at $(printf "0x%X" "${env_start}")"
	fi

	display_alert "${EXTENSION}" "'${base_binfile}' ends at $(printf "0x%X" "${binfile_end}"), clear of env starting at $(printf "0x%X" "${env_start}")" "debug"
}

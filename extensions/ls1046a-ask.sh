...
# Apply ASK kernel patch from the ASK repo
function post_family_config__ask_kernel_patch() {
	...
	local patch_dst="${SRC}/patch/kernel/archive/ls1046a-${KERNEL_MAJOR_MINOR}/003-mono-gateway-ask-kernel_linux_6_12.patch"

	if [[ -f "${patch_src}" ]]; then
		display_alert "ASK extension" "copying kernel patch from ASK repo" "info"
		rm -f "${patch_dst}" 2>/dev/null || true
		run_host_command_logged cp "${patch_src}" "${patch_dst}"
	else
		exit_with_error "ASK kernel patch not found" "${patch_src}"
	fi
}
...
function post_install_kernel_debs__build_ask_modules() {
	...
	# Clean up build dir
	rm -rf "${builddir}/cdx" "${builddir}/fci" "${builddir}/auto-bridge"

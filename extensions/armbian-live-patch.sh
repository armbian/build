function post_family_tweaks__armbian-live-patch() {

display_alert "Installing Armbian Live Patch" "${EXTENSION}" "info"

	cat <<- EOF > "${SDCARD}"/lib/systemd/system/armbian-live-patch.service
	...
	cat <<- EOF > "${SDCARD}"/usr/lib/armbian/armbian-live-patch
	#!/bin/bash
	#
	...
	gpg --keyring /usr/share/keyrings/armbian.gpg --list-keys | grep -q DF00FAF1C577104B50BF1D0093D6889F9F0E78D5
	if [[ \$? != 0 ]]; then
	...
	cat <<- EOF > "${SDCARD}"/etc/apt/apt.conf.d/02-armbian-preupdate

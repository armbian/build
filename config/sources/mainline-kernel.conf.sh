...
function mainline_kernel_decide_version__750_use_torvalds_for_7.2-rc5() {
	if [[ "${KERNELBRANCH}" == 'tag:v7.2-rc5' ]]; then
		display_alert "Using Linus kernel repo for 7.2-rc5" "${KERNELBRANCH}" "warn"
		declare -g KERNELSOURCE="https://github.com/torvalds/linux.git"
		display_alert "mainline-kernel: missing torvalds tag on 7.2-rc5" "Using KERNELSOURCE='${KERNELSOURCE}' for KERNELBRANCH='${KERNELBRANCH}'" "info"
	fi
}

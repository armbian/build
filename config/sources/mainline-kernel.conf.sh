...
function mainline_kernel_decide_version__upstream_release_candidate_number() {
	...
	if [[ "${KERNEL_MAJOR_MINOR}" == "7.3" ]]; then # @TODO: roll over to next MAJOR.MINOR and MAJOR.MINOR-rc1 when it is released
		declare -g KERNELBRANCH="tag:v7.3-rc2"
...
function mainline_kernel_decide_version__750_use_torvalds_for_rc_kernel() {
	if [[ "${KERNELBRANCH}" == 'tag:v7.3-rc2' ]]; then
		display_alert "Using Linus kernel repo for 7.3-rc2" "${KERNELBRANCH}" "warn"
		declare -g KERNELSOURCE="https://github.com/torvalds/linux.git"
		display_alert "mainline-kernel: missing torvalds tag on 7.3-rc2" "Using KERNELSOURCE='${KERNELSOURCE}' for KERNELBRANCH='${KERNELBRANCH}'" "info"
	fi
}

...
InstallOpenMediaVault() {
	...
	sleep 30 \
	# set static network configuration based on DHCP settings \
	dnsnameservers="$(awk -F" " \x27/^nameserver/ {print $2}\x27 /etc/resolv.conf)" \
	dnssearch="$(awk -F" " \x27/^search/ {print $2}\x27 /etc/resolv.conf)" \
	netmask="$(/sbin/ifconfig eth0 | awk -F":" \x27/inet addr:/ {print $4}\x27)" \
	gateway="$(/sbin/ip route | awk -F" " \x27/^default via/ {print $3}\x27)" \
	address="$(/bin/hostname -I | grep -v \x27127.0.0.1\x27)" \
	xmlstarlet ed -L -s /config/system/network/interfaces -t elem -n interface -s /config/system/network/interfaces/interface -t elem -n uuid -v 4fa8fd59-e5be-40f6-a76d-be6a73ed1407 -s /config/system/network/interfaces/interface -t elem -n type -v ethernet -s /config/system/network/interfaces/interface -t elem -n devicename -v eth0 -s /config/system/network/interfaces/interface -t elem -n method -v static -s /config/system/network/interfaces/interface -t elem -n address -v "${address}" -s /config/system/network/interfaces/interface -t elem -n netmask -v "${netmask}" -s /config/system/network/interfaces/interface -t elem -n gateway -v "${gateway}" -s /config/system/network/interfaces/interface -t elem -n dnsnameservers -v "${dnsnameservers}" -s /config/system/network/interfaces/interface -t elem -n dnssearch -v "${dnssearch}" -s /config/system/network/interfaces/interface -t elem -n method6 -v manual /etc/openmediavault/config.xml \
	/usr/sbin/omv-mkconf interfaces \
	sync && reboot' /etc/init.d/firstrun

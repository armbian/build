...
cp -R "${SRC}"/packages/blobs/desktop/debian-skel/skel.all/. "${destination}"/etc/skel
cp -R "${SRC}"/packages/blobs/desktop/debian-skel/skel.lxqt/. "${destination}"/etc/skel

#install cinnamon desktop bar icons
mkdir -p "${destination}"/usr/share/icons/armbian
cp -r "${SRC}"/packages/blobs/desktop/desktop-icons/. "${destination}"/usr/share/icons/armbian

# install wallpapers
mkdir -p "${destination}"/usr/share/backgrounds/armbian
cp -r "${SRC}"/packages/blobs/desktop/desktop-wallpapers/*.jpg "${destination}"/usr/share/backgrounds/armbian/

# install wallpapers
mkdir -p "${destination}"/usr/share/backgrounds/lightdm
cp -r "${SRC}"/packages/blobs/desktop/lightdm-wallpapers/*.jpg "${destination}"/usr/share/backgrounds/lightdm

# install logo for login screen
mkdir -p "${destination}"/usr/share/pixmaps/armbian
cp -r "${SRC}"/packages/blobs/desktop/icons/. "${destination}"/usr/share/pixmaps/armbian

#generate wallpaper list for background changer
mkdir -p "${destination}"/usr/share/lxqt-background-properties
cat <<EOF > "${destination}"/usr/share/lxqt-background-properties/armbian.xml
<?xml version="1.0"?>
<!DOCTYPE wallpapers SYSTEM "lxqt-wp-list.dtd">
<wallpapers>

...
cp -R "${SRC}"/packages/blobs/desktop/debian-skel/skel.icewm/. "${destination}"/etc/skel
...
cp -r "${SRC}"/packages/blobs/desktop/desktop-wallpapers/. "${destination}"/usr/share/backgrounds/armbian/
...
mkdir -p "${destination}"/usr/share/icewm-background-properties
cat <<EOF > "${destination}"/usr/share/icewm-background-properties/armbian.xml
<?xml version="1.0"?>
<!DOCTYPE wallpapers SYSTEM "icewm-wp-list.dtd">
<wallpapers>

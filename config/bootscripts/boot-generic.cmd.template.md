...
|`BOOTSCRIPT_TEMPLATE__DISPLAY_CONSOLE`|The serial device to use for the 'display' console, e.g. `/dev/tty0`.|
|`BOOTSCRIPT_TEMPLATE__SERIAL_CONSOLE`|The serial device to use for the 'serial' console, e.g. `/dev/ttyS1`.|
...
The bootscript template is rendered in `lib/functions/rootfs/distro-agnostic.sh` by the function `render_bootscript_template`. As the display and serial console devices can be defined throughout the build process, the following functions will gather them all and process them accordingly:

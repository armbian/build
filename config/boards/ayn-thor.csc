...
function ayn-thor_is_userspace_supported() {
...
function pre_customize_image__ayn-odin2_alsa_ucm_conf() {
	if ! ayn-thor_is_userspace_supported; then
		return 0
	fi
...
function post_family_tweaks_bsp__ayn-thor_firmware() {
...
function post_family_tweaks__ayn-thor_enable_services() {
	if ! ayn-thor_is_userspace_supported; then
		if [[ "${RELEASE}" != "" ]]; then
...
function post_family_tweaks_bsp__ayn-thor_bsp_firmware_in_initrd() {
	display_alert "Adding to bsp-cli" "${BOARD}: firmware in initrd" "warn"
	declare file_added_to_bsp_destination # Will be filled in by add_file_from_stdin_to_bsp_destination
	# Using odin2portal firmware for now
	add_file_from_stdin_to_bsp_destination "/etc/initramfs-tools/hooks/ayn-thor-firmware" <<- 'FIRMWARE_HOOK'
		#!/bin/bash
		[[ "$1" == "prereqs" ]] && exit 0
		...
		for f in /lib/firmware/qcom/sm8550/ayn/thor/* ; do
			add_firmware "${f#/lib/firmware/}"
		done
